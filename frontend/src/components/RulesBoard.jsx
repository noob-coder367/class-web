...
export default function RulesBoard({
  ...
  const attentionList = useMemo(() => {
    return (ranked.rows || [])
      .map((row) => ({ ...row, status: statusFor(row.score) }))
      .filter((row) => row.status.level !== 'good' && row.status.level !== 'unknown')
  ...
  const filteredViolations = useMemo(() => {
    let list = violations || []
    if (filterDate) {
      list = list.filter((row) => row.date === filterDate)
    }
    if (filterName.trim()) {
      const q = filterName.trim().toLowerCase()
      list = list.filter((row) => String(row.name || '').toLowerCase().includes(q))
    }
    if (filterOffense.trim()) {
      const q = filterOffense.trim().toLowerCase()
      list = list.filter((row) => String(row.offense || '').toLowerCase().includes(q))
    }
    const minP = filterPointsMin === '' ? null : Number(filterPointsMin)
    const maxP = filterPointsMax === '' ? null : Number(filterPointsMax)
    if (Number.isFinite(minP)) {
      list = list.filter((row) => (Number(row.points) || 0) >= minP)
    }
    if (Number.isFinite(maxP)) {
      list = list.filter((row) => (Number(row.points) || 0) <= maxP)
    }
    return list
  }, [violations, filterDate, filterName, filterOffense, filterPointsMin, filterPointsMax])
  ...
  const hasActiveFilters = Boolean(
    filterDate || filterName.trim() || filterOffense.trim() || filterPointsMin !== '' || filterPointsMax !== ''
  )
      ...
      {pane === 'violations' ? (
        <div className="rules-violations">
          ...
          {attentionList.length > 0 ? (
            <section className="rules-attention" aria-label="Diện cần lưu ý">
              <h3 className="rules-attention-title">Diện cần lưu ý</h3>
              <p className="rules-attention-hint">
                Học sinh đang ở mức cảnh báo (không còn ở mức Tốt). Sắp xếp theo điểm thấp nhất trước.
              </p>
              <div className="rules-attention-table-wrap">
                <table className="rules-attention-table">
                  <thead>
                    <tr>
                      <th>Tên học sinh</th>
                      <th>Điểm hiện tại</th>
                  ...
                  <tbody>
                    {attentionList.map((row) => (
                      <tr key={row.id} className={`is-${row.status.level}`}>
                        <td>{row.username}</td>
                        <td className="rules-attention-score">{row.score}</td>
                        <td>
                          <span className={`rank-status-chip is-${row.status.level}`}>
                            {row.status.label}
                          </span>
                        </td>
                      </tr>
          ...
          <div className="rules-filters">
            <h3 className="rules-filters-title">Bộ lọc danh sách vi phạm</h3>
            <div className="rules-filters-grid">
              <label>
                Ngày
                <input
                  type="date"
            ...
            <div className="rules-filters-actions">
              <span className="rules-filters-count">
                Hiển thị {filteredViolations.length}
                {hasActiveFilters ? ` / ${(violations || []).length}` : ''} vi phạm
              </span>
              {hasActiveFilters ? (
                <button type="button" className="rules-filters-clear" onClick={clearFilters}>
                  Xoá bộ lọc
                </button>
              ) : null}
            </div>
          ...
          {!filteredViolations?.length ? (
            <p className="rules-empty">
              {hasActiveFilters ? 'Không có vi phạm khớp bộ lọc' : 'Chưa có vi phạm'}
            </p>
          ) : (
