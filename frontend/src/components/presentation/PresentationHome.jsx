...
export default function PresentationHome({ mode = 'list', presentationId = null, embedded = false }) {
  ...
  // ĐÃ BỎ chặn desktop-only — mọi máy đều tạo / chỉnh sửa được
  ...
  return <div className={`presentation-page${embedded ? ' presentation-page--embedded' : ''}`}><header className="presentation-header"><button type="button" onClick={() => navigate(embedded ? '/vo-lop/thong-bao-chung' : '/')}>← {embedded ? 'Khu vực lớp' : 'Trang chủ'}</button><div><span>CLASSROOM PRESENTATIONS</span><h1>Thuyết trình</h1><p>Tạo, chỉnh sửa và trình chiếu bài thuyết trình</p></div><button className="presentation-primary" type="button" onClick={() => navigate(presentationCreatePath())}>＋ Tạo bài thuyết trình</button></header>{location.state?.editorDesktopOnly ? <p className="presentation-error">Tạo và chỉnh sửa bài thuyết trình chỉ dùng được trên máy tính. Bạn vẫn xem và trình chiếu được trên thiết bị này.</p> : null}<div className="presentation-code-box"><strong>Vào bằng mã bài</strong><input placeholder="Nhập mã bài 6 số" value={lookup} onChange={(e) => setLookup(e.target.value.replace(/\D/g, '').slice(0, 6))} onKeyDown={(e) => { if (e.key === 'Enter') openByCode() }} /><button type="button" onClick={openByCode}>→</button></div>{error ? <p className="presentation-error">{error}</p> : null}{loading ? <div className="presentation-empty">Đang tải…</div> : <div className="presentation-card-grid">{items.map((entry) => <article className="presentation-card" key={entry.id}><div className="presentation-card-cover">{entry.cover ? <img src={entry.cover} alt="" /> : <span>▧</span>}</div><div className="presentation-card-body"><h2>{entry.title}</h2><p>{entry.description || 'Chưa có mô tả'}</p><small>Mã bài: {entry.code} · {entry.slideCount} slide</small><small>Tác giả: {entry.ownerName}</small><div><button type="button" onClick={() => navigate(presentationViewPath(entry.id))}>Thuyết trình</button>{entry.canEdit ? <button type="button" onClick={() => navigate(presentationCreatePath(entry.id))}>Chỉnh sửa</button> : null}</div></div></article>)}</div>}{!items.length && !error && !loading ? <div className="presentation-empty">Chưa có bài thuyết trình. Hãy tạo bài đầu tiên.</div> : null}</div>
}
