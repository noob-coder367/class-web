...
import { PRESENTATION_TEMPLATES, templateById } from './presentationTemplates.js'
...
function CreatePresentation() {
  ...
  const create = async (event) => {
    ...
    try {
      const { item } = await presentationService.createPresentation({ title, description, visibility, password, slides: templateById(template).slides() })
      navigate(presentationCreatePath(item.id))
    } catch (err) { setError(err.message) }
  }
  return <div className="presentation-page presentation-create-page"><header className="presentation-header"><button type="button" onClick={() => navigate(presentationListPath())}>← Thuyết trình</button><div><span>THUYẾT TRÌNH STUDIO</span><h1>Tạo bài thuyết trình</h1><p>Chọn điểm bắt đầu, sau đó hoàn thiện bài trong Editor.</p></div></header><form className="presentation-create-form" onSubmit={create}><label>Tên bài<input value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="Ví dụ: Ô nhiễm môi trường" /></label><label>Mô tả<textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Mô tả ngắn về bài" /></label><fieldset className="presentation-template-picker"><legend>Chọn template</legend><div className="presentation-template-grid">{PRESENTATION_TEMPLATES.map((entry) => <button type="button" key={entry.id} className={template === entry.id ? 'is-selected' : ''} onClick={() => setTemplate(entry.id)}><span className="presentation-template-preview">{entry.id === 'blank' ? '＋' : entry.label.slice(0, 2).toUpperCase()}</span><strong>{entry.label}</strong><small>{entry.description}</small></button>)}</div></fieldset><label>Quyền riêng tư<select value={visibility} onChange={(e) => setVisibility(e.target.value)}><option value="public">Công khai</option><option value="private">Riêng tư</option></select></label>{visibility === 'private' ? <label>Mật khẩu<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={4} required /></label> : null}{error ? <p className="presentation-error">{error}</p> : null}<button className="presentation-primary" type="submit">Mở Presentation Editor</button></form></div>
}

export default function PresentationHome({ mode = 'list', presentationId = null, embedded = false }) {
  const navigate = useNavigate(); const isDesktop = useIsDesktop(); const location = useLocation()
  const [items, setItems] = useState([]); const [item, setItem] = useState(null); const [error, setError] = useState(''); const [lookup, setLookup] = useState(''); const [privatePassword, setPrivatePassword] = useState(''); const [needsPassword, setNeedsPassword] = useState(false); const [loading, setLoading] = useState(mode !== 'create' && mode !== 'editor')
  useEffect(() => { let cancelled = false; if (mode === 'list') { setLoading(true); presentationService.listPresentations().then((data) => { if (!cancelled) setItems(data.items || []) }).catch((err) => { if (!cancelled) setError(err.message) }).finally(() => { if (!cancelled) setLoading(false) }) }; if (mode === 'view' || mode === 'player') { setLoading(true); presentationService.getPresentation(presentationId).then((data) => { if (!cancelled) setItem(data.item) }).catch((err) => { if (!cancelled) { setError(err.message); setNeedsPassword(err.status === 401) } }).finally(() => { if (!cancelled) setLoading(false) }) }; return () => { cancelled = true } }, [mode, presentationId])
  const loadByPassword = async (event) => { event.preventDefault(); setError(''); setLoading(true); try { const data = await presentationService.getPresentation(presentationId, privatePassword); setItem(data.item); setNeedsPassword(false) } catch (err) { setError(err.message); setNeedsPassword(err.status === 401) } finally { setLoading(false) } }
  if ((mode === 'editor' || mode === 'create') && !isDesktop) return <Navigate to={presentationListPath()} replace state={{ editorDesktopOnly: true }} />
  ...
  if ((mode === 'view' || mode === 'player') && !item) return <div className={`presentation-page${embedded ? ' presentation-page--embedded' : ''}`}><p className="presentation-error">{error || 'Không tải được bài thuyết trình.'}</p>{needsPassword ? <form className="presentation-code-box" onSubmit={loadByPassword}><strong>Nhập mật khẩu bài riêng tư</strong><input type="password" value={privatePassword} onChange={(e) => setPrivatePassword(e.target.value)} autoFocus /><button type="submit">Mở bài</button></form> : null}</div>
  if (mode === 'view' && item) return <div className={`presentation-page${embedded ? ' presentation-page--embedded' : ''}`}><header className="presentation-header"><button type="button" onClick={() => navigate(presentationListPath())}>← Thuyết trình</button><div><span>BÀI THUYẾT TRÌNH</span><h1>{item.title}</h1></div><div className="presentation-header-actions"><button type="button" onClick={() => navigate(presentationViewPath(item.id, true))}>Trình chiếu</button>{item.canEdit && isDesktop ? <button type="button" onClick={() => navigate(presentationCreatePath(item.id))}>Chỉnh sửa</button> : null}</div></header><Player item={item} /></div>
  const openByCode = () => { if (lookup.trim()) navigate(presentationViewPath(lookup.trim())) }
  return <div className={`presentation-page${embedded ? ' presentation-page--embedded' : ''}`}><header className="presentation-header"><button type="button" onClick={() => navigate(embedded ? '/vo-lop/thong-bao-chung' : '/')}>← {embedded ? 'Khu vực lớp' : 'Trang chủ'}</button><div><span>CLASSROOM PRESENTATIONS</span><h1>Thuyết trình</h1><p>Tạo, chỉnh sửa và trình chiếu bài thuyết trình</p></div>{isDesktop ? <button className="presentation-primary" type="button" onClick={() => navigate(presentationCreatePath())}>＋ Tạo bài thuyết trình</button> : null}</header>{location.state?.editorDesktopOnly ? <p className="presentation-error">Tạo và chỉnh sửa bài thuyết trình chỉ dùng được trên máy tính. Bạn vẫn xem và trình chiếu được trên thiết bị này.</p> : null}<div className="presentation-code-box"><strong>Vào bằng mã bài</strong><input placeholder="Nhập mã bài 6 số" value={lookup} onChange={(e) => setLookup(e.target.value.replace(/\D/g, '').slice(0, 6))} onKeyDown={(e) => { if (e.key === 'Enter') openByCode() }} /><button type="button" onClick={openByCode}>→</button></div>{error ? <p className="presentation-error">{error}</p> : null}{loading ? <div className="presentation-empty">Đang tải…</div> : <div className="presentation-card-grid">{items.map((entry) => <article className="presentation-card" key={entry.id}><div className="presentation-card-cover">{entry.cover ? <img src={entry.cover} alt="" /> : <span>▧</span>}</div><div className="presentation-card-body"><h2>{entry.title}</h2><p>{entry.description || 'Chưa có mô tả'}</p><small>Mã bài: {entry.code} · {entry.slideCount} slide</small><small>Tác giả: {entry.ownerName}</small><div><button type="button" onClick={() => navigate(presentationViewPath(entry.id))}>Thuyết trình</button>{entry.canEdit && isDesktop ? <button type="button" onClick={() => navigate(presentationCreatePath(entry.id))}>Chỉnh sửa</button> : null}</div></div></article>)}</div>}{!items.length && !error && !loading ? <div className="presentation-empty">Chưa có bài thuyết trình. Hãy tạo bài đầu tiên.</div> : null}</div>
}
